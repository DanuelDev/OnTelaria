...
<div style="max-width: 760px; margin: 120px auto 60px; padding: 0 24px;">
    ...
    <div style="margin-bottom: 2rem;">
        ...
        <h2 style="font-size: 2.4rem; font-weight: 800; color: var(--escuro); margin: 0; line-height: 1.1;">Editar Estadia <span style="color: #bbb; font-size: 1.6rem;">#{{ $estadia->id }}</span></h2>
    </div>
    ...
    <div style="background: white; border-radius: 14px; border: 2px solid var(--primaria); overflow: hidden;">
        ...
        <form action="{{ route('estadias.update', $estadia->id) }}" method="POST" style="padding: 1.8rem 1.5rem;">
            @csrf
            @method('PUT')
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.2rem; margin-bottom: 1.2rem;">
                ...
                <div style="grid-column: 1 / -1;">
                    ...
                    <select name="reserva_id" required style="width: 100%; padding: 0.65rem 0.9rem; border-radius: 9px; border: 1.5px solid {{ $errors->has('reserva_id') ? '#fca5a5' : '#e5e7eb' }}; font-size: 0.92rem; color: var(--escuro); background: white; outline: none; cursor: pointer;">
                        ...
                        @foreach($reservas as $reserva)
                            <option value="{{ $reserva->id }}" {{ old('reserva_id', $estadia->reserva_id) == $reserva->id ? 'selected' : '' }}>
                                Reserva #{{ $reserva->id }}
                                @if($reserva->cliente)
                ...
                <div style="grid-column: 1 / -1;">
                    ...
                    <select name="quarto_id" required style="width: 100%; padding: 0.65rem 0.9rem; border-radius: 9px; border: 1.5px solid {{ $errors->has('quarto_id') ? '#fca5a5' : '#e5e7eb' }}; font-size: 0.92rem; color: var(--escuro); background: white; outline: none; cursor: pointer;">
                        ...
                        @foreach($quartos as $quarto)
                            <option value="{{ $quarto->id }}" {{ old('quarto_id', $estadia->quarto_id) == $quarto->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="date" name="data_checkin" value="{{ old('data_checkin', \Carbon\Carbon::parse($estadia->data_checkin)->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <input type="date" name="data_checkout" value="{{ old('data_checkout', \Carbon\Carbon::parse($estadia->data_checkout)->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <select name="status" required style="width: 100%; padding: 0.65rem 0.9rem; border-radius: 9px; border: 1.5px solid {{ $errors->has('status') ? '#fca5a5' : '#e5e7eb' }}; font-size: 0.92rem; color: var(--escuro); background: white; outline: none; cursor: pointer;">
                        <option value="ativa"     {{ old('status', $estadia->status) == 'ativa'     ? 'selected' : '' }}>Ativa</option>
                        <option value="concluida" {{ old('status', $estadia->status) == 'concluida' ? 'selected' : '' }}>Concluída</option>
                        <option value="cancelada" {{ old('status', $estadia->status) == 'cancelada' ? 'selected' : '' }}>Cancelada</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="valor_estadia" value="{{ old('valor_estadia', $estadia->valor_estadia) }}" required min="0" step="0.01"
                ...
                <div style="grid-column: 1 / -1;">
                    ...
                    <textarea name="observacoes" rows="3"
                              style="width: 100%; padding: 0.65rem 0.9rem; border-radius: 9px; border: 1.5px solid #e5e7eb; font-size: 0.92rem; color: var(--escuro); outline: none; resize: vertical; box-sizing: border-box; font-family: inherit;">{{ old('observacoes', $estadia->observacoes) }}</textarea>
                </div>
            </div>
